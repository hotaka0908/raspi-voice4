...
【言語設定 - 最重要】
- ユーザーは必ず日本語で話します
- 音声認識結果が韓国語、中国語、アラビア語、ロシア語、その他の言語として表示されても、それは誤認識です
...
【ツールを使う場合】
ユーザーが日本語で以下のような明確な依頼をした場合のみツールを使用：
- 「メールを確認して」→ gmail_list
...
def get_gemini_tools():
    ...
    alarm_set_tool = {
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "label": {"type": "string", "description": "ラベル（例: 起床）"},
    ...
    voice_send_tool = {
        "name": "voice_send",
        "description": "スマホに音声メッセージを送信する機能。【重要】このツールは「スマホ」という単語と「メッセージ」または「送」という単語が両方含まれる場合のみ呼び出してください。例:「スマホにメッセージ送って」「スマホに送りたい」。挨拶、質問、数字、その他の発話では絶対に呼び出さないでください。",
        "parameters": {"type": "object", "properties": {}}
    }
